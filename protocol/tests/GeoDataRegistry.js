...
describe("GeoDataRegistry", function () {
  ...
  it("reverts bulk registration when caller lacks oracle role", async function () {
    const { registry, other } = await deployFixture();
    const geoId = buildGeoId(6, 42, [1,2,3,4,5,6]);
    const root = ethers.keccak256(ethers.toUtf8Bytes("batch"));
    const epoch = await registry.currentEpoch();
    const ORACLE_ROLE = await registry.ORACLE_ROLE();
    await expect(
      registry.connect(other).registerGeoBatchBulk(epoch, [geoId], [root], ["cid"])
    ).to.be.revertedWithCustomError(registry, "AccessControlUnauthorizedAccount").withArgs(other.address, ORACLE_ROLE);
  });
  ...
  it("reverts when sensor type is not allowed", async function () {
    const { registry, oracle } = await deployFixture();
    const root = ethers.keccak256(ethers.toUtf8Bytes("batch"));
    const geoId = buildGeoId(6, 42, [1,2,3,4,5,6], 1);
    const epoch = await registry.currentEpoch();
    await expect(
      registry.connect(oracle).registerGeoBatch(epoch, geoId, root, "cid")
    ).to.be.revertedWithCustomError(registry, "SensorTypeNotAllowed");
  });
  ...
  it("reverts when non-manager attempts to pause", async function () {
    const { registry, other } = await deployFixture();
    const MANAGER_ROLE = await registry.MANAGER_ROLE();
    await expect(registry.connect(other).pause())
      .to.be.revertedWithCustomError(registry, "AccessControlUnauthorizedAccount")
      .withArgs(other.address, MANAGER_ROLE);
  });

  it("reverts when non-manager attempts to unpause", async function () {
    const { registry, manager, other } = await deployFixture();
    await registry.connect(manager).pause();
    const MANAGER_ROLE = await registry.MANAGER_ROLE();
  ...
  it("reverts setSensorType when caller lacks manager role", async function () {
    const { registry, other } = await deployFixture();
    const MANAGER_ROLE = await registry.MANAGER_ROLE();
    await expect(registry.connect(other).setSensorType(1, 6))
      .to.be.revertedWithCustomError(registry, "AccessControlUnauthorizedAccount")
      .withArgs(other.address, MANAGER_ROLE);
  });

  it("reverts setEpochMinInterval when caller lacks manager role", async function () {
    const { registry, other } = await deployFixture();
    const MANAGER_ROLE = await registry.MANAGER_ROLE();
    await expect(registry.connect(other).setEpochMinInterval(30))
      .to.be.revertedWithCustomError(registry, "AccessControlUnauthorizedAccount")
      .withArgs(other.address, MANAGER_ROLE);
  });

  it("reverts setEpochMaxDelay when caller lacks manager role", async function () {
    const { registry, other } = await deployFixture();
    const MANAGER_ROLE = await registry.MANAGER_ROLE();
    await expect(registry.connect(other).setEpochMaxDelay(10))
      .to.be.revertedWithCustomError(registry, "AccessControlUnauthorizedAccount")
      .withArgs(other.address, MANAGER_ROLE);
  });
